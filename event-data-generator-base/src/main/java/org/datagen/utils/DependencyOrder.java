...
public final class DependencyOrder {
	...
	private static void insertOrdered(Map<String, List<String>> entries,
			List<String> sorted, List<String> pendings, String entry,
			String dependent) throws CircularDependencyException,
		...
		if (pendings.contains(entry)) {
			...
			throw new CircularDependencyException(CollectionUtils.copyRange(
					pendings, start, pendings.size()));
		}
		...
		if (dependencies != null) {
			for (String dep : dependencies) {
				insertOrdered(entries, sorted, pendings, dep, entry);
			}
		}
	...
	public static List<String> computeOrder(
		...
		for (Map.Entry<String, List<String>> dep : dependencies.entrySet()) {
			System.out.println("   dep " + dep.getKey() + "->"
					+ Arrays.toString(dep.getValue().toArray()));
		}

		List<String> sorted = new ArrayList<>();
		List<String> pendings = new ArrayList<>();

		for (Iterator<Map.Entry<String, List<String>>> iterator = dependencies
				.entrySet().iterator(); iterator.hasNext();) {

			insertOrdered(dependencies, sorted, pendings, iterator.next()
					.getKey(), null);
		}
